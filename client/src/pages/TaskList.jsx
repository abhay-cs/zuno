	...
	return (
		<div className="min-h-screen bg-gray-50 font-sans">
			...
			<header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-40">
				<div className="flex items-center space-x-4">
					...
					<div className="flex items-center space-x-3">
						<img
							src={logo}
							alt="Zuno Logo"
							className="w-auto h-14 px-0 py-2" // adjust size as needed
						/>
				...
				<div className="flex items-center space-x-3">
					<button
						...
						className="hidden lg:inline-flex bg-[#95D9AB] text-[#1E4C3F] px-4 py-2 rounded-lg hover:text-[#1E4C3F] transition-all transform hover:scale-105 shadow-lg hover:shadow-xl font-medium flex items-center space-x-2"
					>
						<Plus size={16} />
			...
			<div className="relative flex overflow-hidden">
				{/* Sidebar */}
				<aside
					className={`fixed top-0 left-0 lg:relative z-50 w-64 h-screen bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out transform
    ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:h-auto`}
				>
					<div className="p-4 mt-16 lg:mt-0">
						...
						<div className="mb-6">
							...
							<div className="space-y-1">
								<div className="flex items-center justify-between p-2 rounded-lg bg-blue-50 border border-[#98DCB0]">
									<div className="flex items-center space-x-3">
										<div className="w-2 h-2 bg-[#98DCB0] rounded-full"></div>
										<span className="text-sm font-medium text-[#1E4C3F]">Personal Tasks</span>
									</div>
									<span className="text-xs text-[#1E4C3F] bg-blue-100 px-2 py-1 rounded-full">{tasks.length}</span>
								</div>
								{/* add list feature later ... */}
							...
							<div className="space-y-1">
								{[
									...
									<button
										...
										className={`w-full flex items-center justify-between p-2 rounded-lg transition-colors ${activeFilter === filter.key
					...
					<div className="max-w-4xl mx-auto">
						...
						<div className="space-y-3">
							{filteredTasks.map(task => (
								<div
									key={task._id}
									className={`bg-white rounded-lg border-l-4 ${getPriorityColor()} shadow-sm hover:shadow-md transition-shadow p-4 ${task.isComplete ? 'opacity-60' : ''
										}`}
								>
									<div className="flex items-start space-x-3">
										<button
											onClick={() => toggleTask(task._id, task.isComplete)}
											className={`mt-1 w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${task.isComplete
							...
							{filteredTasks.length === 0 && (
								<div className="text-center py-12">
									...
									<button
										onClick={() => setShowAddTask(true)}
										className="bg-[#95D9AB] text-[#1E4C3F] px-4 py-2 rounded-lg hover:text-[#1E4C3F] transition-all transform hover:scale-105 shadow-lg hover:shadow-xl font-medium px-4 py-2 rounded-lg"
									>
										Add Task
									</button>
								</div>
			...
			<button
				onClick={() => setShowAddTask(true)}
				className="lg:hidden fixed bottom-6 right-6 bg-[#95D9AB] text-[#1E4C3F] px-4 py-2 rounded-lg shadow-lg hover:text-[#1E4C3F] transition-all transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center justify-center z-30"
			>
				<Plus size={24} />
			</button>
			{/* Add Task Modal */}
				...
				<div className="fixed inset-0 bg-white/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
					<div className="bg-white rounded-lg w-full max-w-md p-6">
						...
						<input
							...
							className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C7EDD8] mb-4"
							autoFocus
						/>
						<div className="flex justify-end space-x-3">
							...
							<button
								...
								className="px-4 py-2 bg-[#95D9AB] text-[#1E4C3F] px-4 py-2 rounded-lg hover:text-[#1E4C3F] transition-all transform hover:scale-105 shadow-lg hover:shadow-xl font-medium rounded-lg"
							>
								Add Task
							</button>
						</div>
